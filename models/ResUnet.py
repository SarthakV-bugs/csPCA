# -*- coding: utf-8 -*-
"""csPCA_Models - ResUNet with Boundary Loss"""
...
class Config:
    ...
    # Model choice: 'resunet' or 'unet'
    model_type = 'resunet'  # Change to 'unet' for original U-Net
    use_pretrained = True  # Use ImageNet pretrained weights for ResUNet
...
class T2WDataset2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # 🔧 FIX: Normalize image to [0, 1] range
            if img_slice.max() > img_slice.min():
                img_slice = (img_slice - img_slice.min()) / (img_slice.max() - img_slice.min())
            else:
                img_slice = np.zeros_like(img_slice, dtype='float32')
                
        except Exception as e:
            print(f"Error reading slice: {e}")
            img_slice = np.random.rand(256, 256).astype('float32')  # 0-1 range
...
# --- ResUNet Architecture (NEW) ---
class ConvBlock(nn.Module):
    """Standard 3x3 convolution with ReLU activation and batch normalization."""
    
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, use_batch_norm=True):
        super().__init__()
...
class ResUNet(nn.Module):
    """U-Net architecture with pre-trained ResNet34 encoder"""
    
    def __init__(self, n_classes=1, in_channels=1, pretrained=True):
        super().__init__()
        ...
        if in_channels != 3:
            ...
            new_conv1.weight.data = original_weights.mean(dim=1, keepdim=True).repeat(1, in_channels, 1, 1)
        ...
        self.dec_conv4 = ConvBlock(256 + 256, 256)
        
        self.upconv3 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        self.dec_conv3 = ConvBlock(128 + 128, 128)
    ...
    def forward(self, x):
        ...
        skip0 = x0  # 64 channels
        
        x1 = self.encoder_maxpool(x0)
        x1 = self.encoder_layer1(x1)
        skip1 = x1  # 64 channels
        
        x2 = self.encoder_layer2(x1)
        skip2 = x2  # 128 channels
        
        x3 = self.encoder_layer3(x2)
        skip3 = x3  # 256 channels
        
        bottleneck = self.encoder_layer4(x3)  # 512 channels
...
class BoundaryDiceFocalLoss(nn.Module):
    ...
    def __init__(self, alpha=0.75, gamma=2, boundary_weight=0.3, 
                 dice_weight=0.4, focal_weight=0.3, smooth=1e-6):
    ...
    def forward(self, inputs, targets):
        ...
        # 2. Dice Loss (per-sample then averaged)
        dice_scores = []
...
def train_one_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for batch_idx, (images, labels, _) in enumerate(tqdm(dataloader, desc="Training")):
        ...
        if batch_idx == 0:
            print(f"\nFirst batch - Image range: [{images.min():.3f}, {images.max():.3f}]")
            print(f"First batch - Label sum: {labels.sum().item()}")
...
def test_model_comprehensive(model, test_loader, device, threshold=0.5):
    """Calculate metrics PER SAMPLE (slice-wise)"""
    model.eval()
...
def main():
    ...
    # Find matching MRI images
    dataset = find_mri_images(config.mri_root, label_map)
    if not dataset:
        print("ERROR: No MRI images found!")
        return
    ...
    if config.model_type == 'resunet':
        print("Using ResUNet with pre-trained ResNet34 encoder")
        model = ResUNet(n_classes=1, in_channels=1, pretrained=config.use_pretrained).to(device)
    else:
    ...
    patience_counter = 0
    checkpoint_path = os.path.join(config.checkpoint_dir, f"best_{config.model_type}_model.pth")
    ...
    # Save results
    results_path = os.path.join(config.checkpoint_dir, f"{config.model_type}_test_results.json")
    results_to_save = {
    ...
    print(f"Visualizations: {config.checkpoint_dir}")
